...
DEBUG = False # enable Component.debug(message) method.
...
class Component(object):
    ...
    def add(self, component, container_label=None):
        ...
        a = 'update_' + self.__class__.__name__
    ...
    def update_parent(self, parent):
        pass
    #def update_<parent class name>(self, parent):.. 
    ...
    def sorted_components(self):
        ...
        for (c,l) in self.components:
            if not (hasattr(c, 'depends') and hasattr(c, 'name')):
                sorted_components.append((c,l))
                continue
        ...
        while dep_map:
            ...
            if dep_map_copy==dep_map:
                self.warnign('%s: detected cyclic dependencies in %r, result is undefined.\n'\
                             % (self.provides, dep_map))
                sorted_components += dep_map.values()
                break
        ...
        for component, container_key in self.components:
            ...
            if container_key == '<IGNORE>':
                ...
                if isinstance(container_key, tuple):
                    assert len(result)==len(container_key),\
                           `len(result),container_key`
                    results = result
                    keys = container_key
                else:
    ...
    def get_container_dict(self):
        ...
        for n in dir(self):
            if n.startswith('_') or not hasattr(self, n):
                continue
            v = getattr(self, n)
    ...
    def get_missing_label(self, label):
        if self.parent is not None:
            d = self.parent.get_container_dict()
            try:
                return d[label]
            except KeyError:
                return self.parent.get_missing_label(label)
        return '%('+label+')s'
    ...
    def evaluate(self, template, **attrs):
        ...
        for label in re.findall(r'(?<=%\()\w+(?=\)s)', template):
            if not d.has_key(label):
                d[label] = self.get_missing_label(label)
        try:
            template = template % d
        except KeyError, msg:
...
class Container(object):
    ...
    >>> print c
    "hey, hoo, bar"
    >>> c = Container(c_block_indent=True)
    ...
    >>> c2 = Container(c_block_indent=True)
    >>> c2 += 'LINE 1 {'
    >>> c2 += c
    >>> c2 += '}'
    >>> print c2
    ...
    def __init__(self,
        ...
        self.use_indent = use_indent or use_firstline_indent or c_block_indent
        ...
        self.c_block_indent = c_block_indent
    ...
    def __radd__(self, indent):
        cpy = self.copy()
        for i in range(len(cpy.list)):
            cpy.list[i] = (indent+cpy.list[i].replace('\n','\n'+indent))
        return cpy
    ...
    def copy(self, mapping=None, **extra_options):
        options = dict(separator=self.separator, prefix=self.prefix, suffix=self.suffix,
                       ...
                       c_block_indent = self.c_block_indent
                       )
