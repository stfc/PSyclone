...
!-----------------------------------------------------------------------------
!
!> @brief Provides field related classes for integer field data of kind int32.
!>
!> @details This is a version of a field object that can hold integer data values
!> of kind int32. It contains both a representation of a field which
...
module field_int32_mod

  use, intrinsic :: iso_fortran_env, only : int32

  use constants_mod,        only: i_def, l_def, str_def, integer_type, &
                                  default_halo_depth
...
!______field_int32_type______________________________________________________

  !> Algorithm layer representation of integer field data of kind int32.
  !>
  ...
  !> alone.
  !>
  type, extends(field_parent_type), public :: field_int32_type
    private
    ...
    integer(int32), pointer :: override_data( : )
  ...
  contains
    ...
    final             :: field_int32_destructor_scalar, &
                         field_int32_destructor_array1d, &
                         field_int32_destructor_array2d

    !> Override default assignment for field_type pairs.
    generic           :: assignment(=) => field_type_assign

  end type field_int32_type
...
!> the pure abstract field class
  ...
  type, extends(pure_abstract_field_type), public :: field_int32_pointer_type
    private
    !> A pointer to a field
    type(field_int32_type), pointer, public :: field_ptr
  contains
    !> Initialiser for a field pointer. May only be called once.
    procedure, public :: initialise => field_int32_pointer_initialiser
    !> Finaliser for a field pointer object
    final :: field_int32_pointer_destructor
  end type field_int32_pointer_type

!______field_int32_proxy_type________________________________________________
  ...
  !>
  type, extends(field_parent_proxy_type), public :: field_int32_proxy_type

    private

    !> Allocatable array of type int32 which holds the values of the field
    integer(int32), public, pointer :: data( : ) => null()
  ...
  end type field_int32_proxy_type

!______end of type declarations_______________________________________________

contains

!______field_int32_type procedures___________________________________________
  ...
  !> field_type
  type(field_int32_proxy_type) function get_proxy(self)
    implicit none
    class(field_int32_type), target, intent(in)  :: self
  ...
  !> Initialise a <code>field_int32_type</code> object.
  !>
  ...
  subroutine field_initialiser(self, &
    ...
    class(field_int32_type), intent(inout)        :: self
    type(function_space_type), pointer, intent(in) :: vector_space
    character(*), optional, intent(in)             :: name
    integer(int32), target, optional, intent(in)     :: override_data( : )
    ...
    ! re-initialisation
    call field_int32_destructor_scalar(self)
  ...
  !> Initialise a int32 field pointer
  !>
  !> @param [in] field_int32_ptr A pointer to the field that is to be
  !>                              stored as a reference
  subroutine field_int32_pointer_initialiser(self, field_ptr)
    implicit none
    class(field_int32_pointer_type) :: self
    type(field_int32_type), pointer :: field_ptr
    self%field_ptr => field_ptr
  end subroutine field_int32_pointer_initialiser

  ! Finaliser for a int32 field pointer
  !
  ...
  subroutine field_int32_pointer_destructor(self)
    implicit none
    type(field_int32_pointer_type), intent(inout) :: self
  end subroutine field_int32_pointer_destructor
  ...
  subroutine copy_field_serial(self, dest, name)
    ...
    implicit none
    class(field_int32_type), target, intent(in)  :: self
    type(field_int32_type),  target, intent(out) :: dest
    character(*),           optional, intent(in)  :: name
  ...
  subroutine field_type_assign(dest, source)
    ...
    implicit none
    class(field_int32_type), intent(in)  :: source
    class(field_int32_type), intent(out) :: dest
  ...
  subroutine field_int32_destructor_scalar(self)

    implicit none

    type(field_int32_type), intent(inout) :: self

    call self%field_final()

  end subroutine field_int32_destructor_scalar

  !> Finalizer for a 1d array of <code>field_type</code> instances.
  subroutine field_int32_destructor_array1d(self)
  ...
  end subroutine field_int32_destructor_array1d

  !> Finalizer for a 2d array of <code>field_type</code> instances.
  subroutine field_int32_destructor_array2d(self)
  ...
  end subroutine field_int32_destructor_array2d
  ...
  subroutine get_write_behaviour(self, write_behaviour)
    ...
    class(field_int32_type), intent(in)               :: self
  ...
  subroutine write_field(this, field_name)
    ...
    class(field_int32_type),   intent(in) :: this
  ...
  subroutine read_field( self, field_name)
    ...
    class( field_int32_type ),  target, intent(inout) :: self
    character(len=*),                    intent(in)    :: field_name

    type( field_int32_proxy_type )                    :: tmp_proxy
  ...
  subroutine halo_exchange( self, depth )
    ...
    class( field_int32_proxy_type ), target, intent(inout) :: self
  ...
  function get_sum(self) result (answer)
    ...
    class(field_int32_proxy_type), intent(in) :: self
