...
module operator_int32_mod
  ...
  !>
   type, public, extends(operator_parent_type) :: operator_int32_type
    private
    !> Allocatable array of kind int32 which holds the values of the operator
    real(kind=int32), allocatable :: local_stencil( :, :, : )
    !> Size of the outermost dimemsion of the local_stencil array, equal to
    !! ncell*nlayers
    integer(i_def) :: ncell_3d
  contains
    !> Initialise an operator object
    procedure, public :: initialise => operator_int32_initialiser
    !> Function to get a proxy with public pointers to the data in a
    !! operator_int32_type.
    procedure, public :: get_proxy => get_proxy_operator_int32
    !> Deep copy methods
    procedure, private :: operator_int32_type_deep_copy
    procedure, public  :: deep_copy => operator_int32_type_deep_copy
    !> Destroys object
    procedure, public :: operator_final
    ! Finalizer for the object
    final :: operator_int32_destructor
 end type operator_int32_type
 ...
 !>
 type, public, extends(operator_parent_proxy_type) :: operator_int32_proxy_type
    private
    !> Allocatable array of type real which holds the values of the operator
    real(kind=int32), public, pointer :: local_stencil( :, :, : )
    !> size of the outermost dimension
    integer(i_def), public :: ncell_3d
  contains
    final :: operator_int32_proxy_destructor
 end type operator_int32_proxy_type

!______end of type declarations_______________________________________________

contains

  !> Initialise an <code>operator_int32_type</code> object.
  !>
  ...
  !>
  subroutine operator_int32_initialiser( self, fs_to, fs_from )
    ...
    implicit none
    class(operator_int32_type), target, intent(inout) :: self
  ...
  end subroutine operator_int32_initialiser

  !> Function to create a proxy with access to the data in the operator_int32_type.
  !>
  !> @return The proxy type with public pointers to the elements of
  !> operator_int32_type
  type(operator_int32_proxy_type ) function get_proxy_operator_int32(self)
    implicit none
    class(operator_int32_type), target, intent(in)  :: self

    ! Call the routine that initialises the proxy for data held in the parent
    call self%operator_parent_proxy_initialiser(get_proxy_operator_int32)

    get_proxy_operator_int32 % local_stencil  => self % local_stencil
    get_proxy_operator_int32 % ncell_3d       =  self%ncell_3d
  end function get_proxy_operator_int32

  !> @brief Copies the current operator into a new returned object
  !> @return The copies operator type
  function operator_int32_type_deep_copy(self) result(other)
    implicit none
    class(operator_int32_type), intent(in) :: self
    type(operator_int32_type) :: other
    ! make field_vector
  ...
  end function operator_int32_type_deep_copy

  !>@brief Destroys the operator type
  pure subroutine operator_final(self)
    implicit none
    class(operator_int32_type), intent(inout) :: self
  ...
  elemental subroutine operator_int32_destructor(self)
    implicit none
    type(operator_int32_type), intent(inout) :: self
    call self%destroy_operator_parent()
    call self%operator_final()
  end subroutine operator_int32_destructor

  !>@brief Destroy the operator proxy
  elemental subroutine operator_int32_proxy_destructor(self)
    implicit none
    type(operator_int32_proxy_type), intent(inout) :: self
    call self%destroy_operator_parent_proxy()
    nullify(self%local_stencil)
  end subroutine operator_int32_proxy_destructor

end module operator_int32_mod
