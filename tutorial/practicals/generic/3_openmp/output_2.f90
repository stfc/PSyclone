...
&e.g.:',/'   " // "JPI=100 JPJ=100 JPK=30 IT=10 ./tra_adv.exe')")
    ...
    WRITE(*, "('Domain is ', I4, ' x', I4, ' grid points with ', I3,  ' vertical levels')") jpi, jpj, jpk
...
&ABS(zwx(ji,jj,jk)))
      ...
      zdt = 1
      zbtr = 1.
      !$omp parallel do default(shared) private(ji,jj,jk,z0w,zalpha,zw,zzwx,zzwy) schedule(auto)
      do jk = 1, jpk - 1, 1
        do jj = 2, jpj - 1, 1
          do ji = 2, jpi - 1, 1
      ...
      !$omp end parallel do
      zbtr = 1.
      !$omp parallel do default(shared) private(ji,jj,jk,ztra) schedule(auto)
      do jk = 1, jpk - 1, 1
        do jj = 2, jpj - 1, 1
          do ji = 2, jpi - 1, 1
