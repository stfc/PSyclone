module tra_adv_mod
  ...
  subroutine tra_adv()
    ...
    type(profile_PSyDataType), save, target :: profile_psy_data_4
    type(profile_PSyDataType), save, target :: profile_psy_data_5
    type(profile_PSyDataType), save, target :: profile_psy_data_6
    type(profile_PSyDataType), save, target :: profile_psy_data_7
    type(profile_PSyDataType), save, target :: profile_psy_data_8
    type(profile_PSyDataType), save, target :: profile_psy_data_9
    type(profile_PSyDataType), save, target :: profile_psy_data_10
    type(profile_PSyDataType), save, target :: profile_psy_data_11
    type(profile_PSyDataType), save, target :: profile_psy_data_12
    type(profile_PSyDataType), save, target :: profile_psy_data_13
...
&e.g.:',/'   " // "JPI=100 JPJ=100 JPK=30 IT=10 ./tra_adv.exe')")
    ...
    r = jpi * jpj * jpk
    CALL profile_psy_data % PreStart("kloop", "1", 0, 0)
    do jk = 1, jpk, 1
      do jj = 1, jpj, 1
        do ji = 1, jpi, 1
    ...
    do jj = 1, jpj, 1
      ...
      enddo
    enddo
    CALL profile_psy_data_1 % PreStart("kloop", "2", 0, 0)
    do jk = 1, jpk, 1
      rnfmsk_z(jk) = jk / jpk
    enddo
    CALL profile_psy_data_1 % PostEnd
    do jt = 1, it, 1
      CALL profile_psy_data_2 % PreStart("kloop", "3", 0, 0)
      do jk = 1, jpk, 1
        do jj = 1, jpj, 1
          do ji = 1, jpi, 1
        ...
        enddo
      enddo
      CALL profile_psy_data_2 % PostEnd
      zwx(:,:,jpk) = 0.e0
      zwy(:,:,jpk) = 0.e0
      CALL profile_psy_data_3 % PreStart("kloop", "4", 0, 0)
      do jk = 1, jpk - 1, 1
        do jj = 1, jpj - 1, 1
          do ji = 1, jpi - 1, 1
      ...
      zslpx(:,:,jpk) = 0.e0
      zslpy(:,:,jpk) = 0.e0
      CALL profile_psy_data_4 % PreStart("kloop", "5", 0, 0)
      do jk = 1, jpk - 1, 1
        do jj = 2, jpj, 1
          do ji = 2, jpi, 1
        ...
        enddo
      enddo
      CALL profile_psy_data_4 % PostEnd
      CALL profile_psy_data_5 % PreStart("kloop", "6", 0, 0)
      do jk = 1, jpk - 1, 1
        do jj = 2, jpj, 1
          do ji = 2, jpi, 1
...
&ABS(zwy(ji,jj,jk)))
        ...
        enddo
      enddo
      CALL profile_psy_data_5 % PostEnd
      CALL profile_psy_data_6 % PreStart("kloop", "7", 0, 0)
      do jk = 1, jpk - 1, 1
        zdt = 1
        do jj = 2, jpj - 1, 1
          do ji = 2, jpi - 1, 1
        ...
        enddo
      enddo
      CALL profile_psy_data_6 % PostEnd
      CALL profile_psy_data_7 % PreStart("kloop", "8", 0, 0)
      do jk = 1, jpk - 1, 1
        do jj = 2, jpj - 1, 1
          do ji = 2, jpi - 1, 1
            zbtr = 1.
        ...
        enddo
      enddo
      CALL profile_psy_data_7 % PostEnd
      zwx(:,:,1) = 0.e0
      zwx(:,:,jpk) = 0.e0
      CALL profile_psy_data_8 % PreStart("kloop", "9", 0, 0)
      do jk = 2, jpk - 1, 1
        zwx(:,:,jk) = tmask(:,:,jk) * (mydomain(:,:,jk - 1) - mydomain(:,:,jk))
      enddo
      CALL profile_psy_data_8 % PostEnd
      zslpx(:,:,1) = 0.e0
      CALL profile_psy_data_9 % PreStart("kloop", "10", 0, 0)
      do jk = 2, jpk - 1, 1
        do jj = 1, jpj, 1
          do ji = 1, jpi, 1
        ...
        enddo
      enddo
      CALL profile_psy_data_9 % PostEnd
      CALL profile_psy_data_10 % PreStart("kloop", "11", 0, 0)
      do jk = 2, jpk - 1, 1
        do jj = 1, jpj, 1
          do ji = 1, jpi, 1
...
&ABS(zwx(ji,jj,jk)))
        ...
        enddo
      enddo
      CALL profile_psy_data_10 % PostEnd
      zwx(:,:,1) = pwn(:,:,1) * mydomain(:,:,1)
      zdt = 1
      zbtr = 1.
      CALL profile_psy_data_11 % PreStart("kloop", "12", 0, 0)
      do jk = 1, jpk - 1, 1
        do jj = 2, jpj - 1, 1
          do ji = 2, jpi - 1, 1
        ...
        enddo
      enddo
      CALL profile_psy_data_11 % PostEnd
      zbtr = 1.
      CALL profile_psy_data_12 % PreStart("kloop", "13", 0, 0)
      do jk = 1, jpk - 1, 1
        do jj = 2, jpj - 1, 1
          do ji = 2, jpi - 1, 1
        ...
        enddo
      enddo
      CALL profile_psy_data_12 % PostEnd
    enddo

    ! PSyclone CodeBlock (unsupported code) reason:
    !  - Unsupported statement: Open_Stmt
    OPEN(UNIT = 4, FILE = 'output.dat', FORM = 'formatted')
    CALL profile_psy_data_13 % PreStart("kloop", "14", 0, 0)
    do jk = 1, jpk - 1, 1
      do jj = 2, jpj - 1, 1
        do ji = 2, jpi - 1, 1
          ! PSyclone CodeBlock (unsupported code) reason:
      ...
      enddo
    enddo
    CALL profile_psy_data_13 % PostEnd
