...
we have done in earlier tutorial sections (`schedule.walk(Loop)`) we
...
fields in the loop are not discontinuous (i.e. it is not definitely
discontinuous so is or might be continuous).
...
see the various loop types in the psy-layer PSyIR that is output to
the screen.
...
Take a look at the generated coloured loops. They call LFRic
