...
def test_evaluate_condition_bool(fortran_reader, fortran_writer):
    ...
    st.add(x_sym)
    st.add(y_sym)
    #node = fortran_reader.psyir_from_expression("x", st)
    #assert eval.evaluate(node)
    #assert eval.evaluate_with_sympy(node)
    #node = fortran_reader.psyir_from_expression("y",st)
