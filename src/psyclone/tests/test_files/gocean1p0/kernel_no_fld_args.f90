module kernel_no_fld_args
  ...
  type, extends(kernel_type) :: bc_ssh
     type(go_arg), dimension(2) :: meta_args =              &
          (/ go_arg(GO_READWRITE, GO_I_SCALAR, GO_POINTWISE),  &
...
contains
  ...
  subroutine bc_ssh_code(ji, jj, istep, tmask)
    ...
    if(tmask(ji,jj) > 0) istep = istep + 1

  end subroutine bc_ssh_code

end module kernel_no_fld_args
