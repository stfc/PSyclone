...
module reordered_compute_cu_mod
  use argument_mod
  use field_mod
  use grid_mod
  use kind_params_mod
...
contains
  ...
  !! compute_cu_code().
  subroutine invoke_compute_cu(pfld, ufld, cufld)
    implicit none
    ...
    do J=cufld%internal%ystart, cufld%internal%ystop
       do I=cufld%internal%xstart, cufld%internal%xstop
          ...
          call compute_cu_code(i, j, pfld%data, ufld%data, cufld%data)
       end do
    end do
  ...
  subroutine compute_cu_code(i, j, p, u, cu)
    implicit none
...
end module reordered_compute_cu_mod
