...
from psyclone.psyir.symbols import (
    DataTypeSymbol, ScalarType, StructureType, SymbolTable)
...
def test_metadata_from_psyir(fortran_reader):
    ...
    # Metadata lookup remains case-insensitive.
    assert GOceanKernelMetadata.create_from_kernel_psyir(
        root, "CoMpUtE_Cu").metadata == metadata


def test_gocean_container_create():
    """Preserve direct construction and metadata access for the specialised
    GOcean container.
    """
    metadata = GOceanKernelMetadata.create_from_psyir(
        FortranReader().psyir_from_source(
            f"module metadata_mod\n{METADATA}\nend module metadata_mod\n"))
    container = GOceanContainer("name", None)
    assert container.name == "name"
    assert container.metadata is None

    container = GOceanContainer.create(
