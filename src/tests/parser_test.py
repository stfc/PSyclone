...
class TestParserGOcean1p0:
    ...
    def test_kernels_different_grid_offsets_one_invoke(self):
        with pytest.raises(ParseError):
            parse(os.path.join(os.path.dirname(os.path.abspath(__file__)),"test_files","gocean1p0","different_grid_offsets_one_invoke.f90"),api="gocean1.0")
    ...
    def test_kernels_different_grid_offsets_two_invokes(self):
        ...
        with pytest.raises(ParseError):
            parse(os.path.join(os.path.dirname(os.path.abspath(__file__)),"test_files","gocean1p0","different_grid_offsets_two_invokes.f90"),api="gocean1.0")
