...
class TestAlgGenClassDynamo0p3(object):
    ...
    def test_single_function_invoke(self):
        ...
        gen = str(alg)
        assert(
            "USE psy_single_invoke, ONLY: invoke_0_testkern_type" in gen and
            "CALL invoke_0_testkern_type(a, f1, f2, m1, m2)" in gen)
    ...
    def test_multi_function_invoke(self):
        ...
        alg, _ = generate(
            os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "test_files", "dynamo0p3", "1.2_multi_invoke.f90"),
            api="dynamo0.3")
        gen = str(alg)
        assert(
            "USE psy_multi_invoke, ONLY: invoke_0" in gen and
            "CALL invoke_0(a, f1, f2, m1, m2, f3)" in gen)
    ...
    def test_multiple_qr_per_invoke(self):
        ...
        alg, _ = generate(
            os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "test_files", "dynamo0p3",
                         "6_multiple_QR_per_invoke.f90"),
            api="dynamo0.3")
        gen = str(alg)
        assert(
            "USE psy_multi_qr_per_invoke, ONLY: invoke_0" in gen and
            "CALL invoke_0(f1, f2, f3, f4, f0, qr0, qr1)" in gen)
    ...
    def test_qr_argnames(self):
        ...
        alg, _ = generate(
            os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "test_files", "dynamo0p3", "7_QR_field_array.f90"),
            api="dynamo0.3")
        gen = str(alg)
        assert(
            "USE psy_qr_field_array, ONLY: invoke_0" in gen and
            "CALL invoke_0(f1, f2, f3, f4, f0, qr0(i, j), qr0(i, j + 1), "
            "qr1(i, k(l)))" in gen)


class TestAlgGenClassGungHoProto(object):
    ...
    def test_single_invoke(self):
        ...
        alg, _ = generate(
            os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "test_files", "gunghoproto", "1_single_function.f90"),
            api="gunghoproto")
        gen = str(alg)
        assert(
            "USE psy_single_function, ONLY: invoke_testkern_type" in gen and
            "CALL invoke_0_testkern_type(f1, f2, m1)" in gen)
    ...
    def test_multi_invoke(self):
        ...
        gen = str(alg)
        assert(
            "USE psy_two_functions_shared_arguments, ONLY: invoke_0" in gen and
            "CALL invoke_0(f1, f2, m1, m2, f3, m3)" in gen)
    ...
    def test_multi_invoke_named(self):
        ...
        alg, _ = generate(
            os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "test_files", "gunghoproto", "4_explicit_name.f90"),
            api="gunghoproto")
        gen = str(alg)
        assert(
            "USE psy, ONLY: invoke_multikern_kern" in gen and
            "CALL invoke_0_multikern_kern(f1, f2, f3, m1, m3, m2)" in gen)
    ...
    def test_single_set(self):
        ...
        alg, psy = generate(
            os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "test_files", "gunghoproto", "1_single_set.f90"),
            api="gunghoproto")
        assert(
            str(alg).find("CALL invoke_0(one)") != -1 and
