...
replaced by '_' characters) is used to name the corresponding
...
within an Algorithm source file. In the future it is intended that
this name will allow invoke-specific optimisations to be applied as
well as enabling more readable profiling output. It may also be used
to instruct PSyclone to just generate a single subroutine to implement all
invokes that share the same label.
