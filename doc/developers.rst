...
and --f90 and --f90flags), e.g.::
  ...
  > py.test --compileopencl --f90=openclcompiler --f90flags="--opencl-flags"
...
.mod files. For dynamo0.3 a stub implemenation of the required functions
from the LFRic infrastructure is included in 
``tests/test_files/dynamo0p3/infrastructure``. When compilation tests
are requested, the stub files are compiled to create the .mod files. If you 
compile in parallel (see :ref:`parallel_execution` section) each
process will compile its own version of the files to avoid race conditions.
This happens only once per process in each test session.

For the gocean1.0 API a complete copy of the dl_esm_inf library is included in
``external/dl_esm_inf``. It is required that this library is compiled
before compilation tests are done, which can be done by using ``make`` in
``external/dl_esm_inf/finite_difference``. All PSyclone gocean1.0 compilation
tests will reference those files.
