...
class PSyIROneLineDirective(Directive):
    """Render a constant using pprint.pformat and insert into the document"""
    required_arguments = 1

    def run(self):
        module_path, member_name = self.arguments[0].rsplit('.', 1)
        node = getattr(import_module(module_path), member_name)
        node_name = node._text_name
        node_format = node. _children_valid_format
        # import pdb; pdb.set_trace()
        text = node_name + " := " + node_format
        link = "https://psyclone-ref.readthedocs.io/en/latest/autogenerated/"
        link += module_path + ".html#" + self.arguments[0]

        paragraph = nodes.paragraph('', '')
        reflink = nodes.reference(text, text, internal=False, refuri=link)
        paragraph.append(reflink)

        return [
                addnodes.desc_content('', paragraph)
        ]


def setup(app):
    app.add_directive('psyir_oneline', PSyIROneLineDirective)
