...
subroutine poly1d_reconstruction_code( nlayers,        &
  ...
  map1d(stencil+depth, face) = stencil + mod(face+1,2)*stencil_depth + mod(stencil,2)*order
  ...
  do f = 1,nfaces
     ...
     reconstruction(df:df+nl) = 0.0_r_tran
  end do
...
subroutine compute_reconstruction(p, f, order, reconstruction, recon_cell, coeff, coeff_cell, &
  ...
  df = recon_cell + (f-1)*(nl+1)
  ijp = (p - 1 + (f-1)*(order+1)) + coeff_cell
  do k = 0, nl
     reconstruction(df + k) = reconstruction(df + k) &
                               + tracer( tracer_cell + k )*coeff( ijp )
  end do
