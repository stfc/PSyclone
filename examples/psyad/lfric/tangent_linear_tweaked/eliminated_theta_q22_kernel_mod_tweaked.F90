...
subroutine eliminated_theta_q22_code(cell, nlayers, ncell_3d,    &
  ...
  real(kind=r_def) :: tmp_result1, tmp_result2
  ...
  do k = 0, nlayers-1
    ...
    do qp2 = 1, nqp_v
      do qp1 = 1, nqp_h
        ...
        do df2 = 1, ndf_w2
          tmp_result1 = 0.0_r_def
          do i = 1, 3
            tmp_result1 = tmp_result1 + grad_theta(i) * basis_w2(i,df2,qp1,qp2)
          end do
          grad_term = wqp_h(qp1) * wqp_v(qp2) * const                     &
                     *tmp_result1 &
                     *grad_exner/dj(qp1,qp2)
          do df = 1, ndf_w2
            tmp_result2 = 0.0_r_def
            do i = 1, 3
              tmp_result2 = tmp_result2 + basis_w2(i,df,qp1,qp2) * grad_term(i)
            end do
