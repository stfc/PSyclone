...
subroutine apply_variable_hx_code_r_double(cell,                    &
  ...
  real(kind=r_double), dimension(ndf_w3) :: lhs_e, p_e, lhs_e_div, lhs_e_m3, lhs_e_p3t
  ...
  do k = 0,nlayers-1
    ...
    lhs_e_div = matmul(div(:,:,ik),x_e)
    lhs_e_p3t = matmul(p3t(:,:,ik),t_e)
    lhs_e_m3 = matmul(m3(:,:,ik),p_e)
    lhs_e = lhs_e_m3 + sgn*lhs_e_div + sgn*lhs_e_p3t
    !RF lhs_e = matmul(m3(:,:,ik),p_e) + sgn*(matmul(div(:,:,ik),x_e) + matmul(p3t(:,:,ik),t_e))
    do df = 1,ndf_w3
...
subroutine apply_variable_hx_code_r_single(cell,                    &
  ...
  real(kind=r_single), dimension(ndf_w3) :: lhs_e, p_e, lhs_e_div, lhs_e_m3, lhs_e_p3t
