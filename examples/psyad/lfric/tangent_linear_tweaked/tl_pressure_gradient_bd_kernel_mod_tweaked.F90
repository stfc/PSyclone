...
contains
  ...
  subroutine tl_pressure_gradient_bd_code( nlayers,         &
    ...
    do k = 0, nlayers-1
      ...
      do face = 1, nfaces_re_h
         ...
         ! stencil_w3_map(:,1) == map_w3(:)
         !call compute_update( &
         !     face, k, ls_exner, exner, map_w3(:), stencil_w3_map(:,face+1), ndf_w3, undf_w3, &
         !     ls_theta, theta, ls_moist_dyn_gas, ls_moist_dyn_tot, ls_moist_dyn_fac, &
         !     map_wtheta, ndf_wtheta, undf_wtheta, &
         !     r_u_bd, map_w2, ndf_w2, undf_w2, cp)
  ...
  subroutine compute_update(face, k, ls_exner, exner, map_w3, ndf_w3, undf_w3, stencil_w3_map, &
       ls_theta, theta, ls_moist_dyn_gas, moist_dyn_gas, ls_moist_dyn_tot, moist_dyn_tot, ls_moist_dyn_fac, &
       map_wtheta, ndf_wtheta, undf_wtheta,&
       r_u_bd, map_w2, ndf_w2, undf_w2, cp, opposite_face, outward_normals_to_horizontal_faces,&
       w2_basis_face, w3_basis_face, wtheta_basis_face, nfaces_qr, wqp_f, nqp_f)
    integer, intent(in) :: face, k
    integer, intent(in) :: ndf_w2, undf_w2, ndf_w3, undf_w3, ndf_wtheta, undf_wtheta, nqp_f, nfaces_qr
    integer(kind=i_def), dimension(ndf_w2),     intent(in) :: map_w2
    integer(kind=i_def), dimension(ndf_w3),     intent(in) :: map_w3, stencil_w3_map
    integer(kind=i_def), dimension(ndf_wtheta), intent(in) :: map_wtheta
    real(kind=r_def), dimension(undf_w3),     intent(in)    :: exner
    real(kind=r_def), dimension(undf_w3),     intent(in)    :: ls_exner
    real(kind=r_def), dimension(undf_wtheta), intent(in)    :: ls_theta, theta
    real(kind=r_def), dimension(undf_wtheta), intent(in)    :: ls_moist_dyn_gas, moist_dyn_gas, &
                                                               ls_moist_dyn_tot, moist_dyn_tot, &
                                                               ls_moist_dyn_fac
    real(kind=r_def),                         intent(in)    :: cp
    real(kind=r_def), dimension(undf_w2),     intent(inout) :: r_u_bd
    ...
    real(kind=r_def) :: ls_exner_av
    real(kind=r_def) :: ls_theta_v_at_fquad
    integer(kind=i_def) :: df, qp, face_next
    ...
    do df = 1, ndf_w3
       ...
       ls_exner_next_e(df) = ls_exner( stencil_w3_map(df) + k )
    end do
    do df = 1, ndf_wtheta
    ...
    do df = 1, ndf_w3
       ...
       exner_next_e(df) = exner( stencil_w3_map(df) + k )
    end do
    do df = 1, ndf_wtheta
    ...
    do qp = 1, nqp_f
       ...
       do df = 1, ndf_w2
          ...
          r_u_bd( map_w2(df) + k ) =  r_u_bd( map_w2(df) + k ) + wqp_f(qp,face) * bdary_term
       end do
